...
get_ancunaware_localanc=function(t.NUMA,t.L,t.G,t.nchrno,t.noanc_gfbs,t.Mu,t.alpha){ 
  ...
  for (ch in 1:t.nchrno) {
    ancunaware_localanc[[ch]]<-array(NaN,c(t.L,t.NUMA,t.G[ch]))
    for (k in 1:t.NUMA) {
...
get_ancunaware_gfbs=function(t.NUMA,t.L,t.G,t.nchrno,t.noanc_gfbs,t.Mu,t.alpha){ 
  ancunaware_gfbs=list()
  for (ch in 1:t.nchrno){
    ancunaware_gfbs[[ch]]=list()
    for (k in 1:t.NUMA) {
      ind=as.integer((k+1)/2)
      ancunaware_gfbs[[ch]][[k]]<-matrix(NaN,t.G[ch],t.L*kLL)
      for (ll in 1:kLL) 
	for (a in 1:t.L) {
