...
if (NCHR==1)
{
  ...
  for (ch in 1:nchrno)
    for (k in 1:NUMA)
    {
      ...
      if (PNG)
	png(file=paste0("Localanc/", target, "_HAP_", NN, "_", k,".",chrnos[ch],".png"),width=1920,height=960)
...
if (NCHR==2)
{
  ...
  for (ch in 1:nchrno)
    for (ind in 1:(NUMI))
    {
      hap<-c(ind*2-1,ind*2)
      if (PNG)
	png(file=paste0("Localanc/", target, "_DIP_", NN, "_", ind,".",chrnos[ch],".png"),width=1920,height=960)
