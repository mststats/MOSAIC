...
4 sets of parameters inferred via EM:
	...
	3. M (copying matrix i.e. M[i,j] is  prob. of picking a hap from group i given latent ancestry j; dimension is KxL where K is #donorpops) 
...
Setting s.M=0 would turn off MCMC phasing until after EM convergence and this is the default. 
...
(2) ord.M<-plot_M(M,MODE="copy",cexa=2) # to look at the copying matrix 
(3) ord.M<-plot_M(M,MODE="scaled",cexa=2) # to look at the copying matrix scaled by #donor haps in each panel. 
