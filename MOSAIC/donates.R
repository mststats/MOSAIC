...
create_donates<-function(getswitches,ch,ind,t.umatch,t.maxmatchsize,t.dw,t.tw,t.gobs,t.flips,t.kLL,t.Mu,t.rho,t.theta,HPC,prethin=F,t.max.donors,t.NUMP)
{
  THIN=F
  ...
  t.ndonors<-rep(t.NUMP,G[ch]) # too large; should limit this really
  ...
  for (h in 1:H) { 
    noanc_fors[[h]]<-rep(0,G[ch]*t.NUMP)
    noanc_sumfors[[h]]<-matrix(0,G[ch],L)
    noanc_backs[[h]]<-rep(0,G[ch]*t.NUMP)
  ...
  {
    k=hap[h]
    cppforward(k,NUMA,t.NUMP,THIN,t.NUMP,kLL,L,0,G[ch],G[ch],noanc_transitions,t.umatch,t.maxmatchsize,d.w[[ch]],t.w[[ch]],t.gobs,noanc_mutmat,maxmiss,noanc_initProb[h,],
	       label,t.ndonors,t.donates,t.donatesl,t.flips,noanc_fors[[h]],noanc_sumfors[[h]],noanc_scalefactor[[h]])
    cppbackward(k,NUMA,t.NUMP,THIN,t.NUMP,L,0,G[ch],G[ch],noanc_transitions,t.umatch,t.maxmatchsize,d.w[[ch]],t.w[[ch]],t.gobs,noanc_mutmat,maxmiss,
		label,t.ndonors,t.donates,t.donatesr,t.flips,noanc_backs[[h]],noanc_scalefactorb[[h]])
    probmass<-probmass+cppforback(t.NUMP,THIN,t.NUMP,L,G[ch],t.ndonors,t.donates,noanc_fors[[h]],noanc_backs[[h]]) # 1 as first argument b/c using all now
    if (getswitches) 
      switches[[h]]<-t(matrix(cppswitches(h,NUMA,t.NUMP,THIN,t.NUMP,G[ch],NL,label,noanc_sumfors[[h]],noanc_backs[[h]],
					  noanc_transitions,t.flips,noanc_mutmat,maxmiss,t.umatch,t.maxmatchsize,t.dw,t.tw,t.gobs,t.ndonors,t.donates)$switches,t.NUMP))
  }
  if (t.max.donors==t.NUMP & getswitches) 
  ...
  if (NUMA>1)
    for (h in 1:2) # both haps fors and backs must be calculated before the next line is called
      probmass<-probmass+cppforback(t.NUMP,THIN,t.NUMP,L,G[ch],t.ndonors,t.donates,noanc_fors[[h]],noanc_backs[[h+ifelse(h%%2,1,-1)]]) # 1 as first argument b/c using all now
  ...
  t.donatesl<-t.donatesr<-matrix(0L,t.max.donors,G[ch]) # re-sizing and 0s first
