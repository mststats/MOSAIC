...
It is not necessary that any of the panels are good surrogates for the unseen mixing populations as MOSAIC will infer parameters controlling:

1. Recombination rates before and after admixture ($\rho$ and $\Pi$ respectively). 

1. The stochastic relationship between panels and ancestral populations ($\mu$). 
...
```
to creates a version of the copying matrix $\mu$ plot that uses bar colour densities rather than bar lengths 
