...
$ PI_command='unlist\(MOSAIC::extract_paras\(MOSAIC::extract_log\(\"$logfile\"\)\)\$PI\)'
$ PI=`eval Rscript -e $PI_command | cut -d " " -f 3-`
$ Mu_command='c\(MOSAIC::extract_paras\(MOSAIC::extract_log\(\"$logfile\"\)\)\$Mu\)'
$ Mu=`eval Rscript -e $Mu_command | cut -d " " -f 2-`
$ rho_command='c\(MOSAIC::extract_paras\(MOSAIC::extract_log\(\"$logfile\"\)\)\$rho\)'
$ rho=`eval Rscript -e $rho_command | cut -d " " -f 2-`
$ theta_command='c\(MOSAIC::extract_paras\(MOSAIC::extract_log\(\"$logfile\"\)\)\$theta\)'
$ theta=`eval Rscript -e $theta_command | cut -d " " -f 2-`
