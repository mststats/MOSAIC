run_mosaic=function(target,datasource,chrnos,L,NUMA,pops=NULL,REPS=0,GpcM=60,PHASE=TRUE,nl=1000,max.donors=100,prop.don=0.99,
  ...
  if (doFst) {
    ...
    if (target=="simulated")
      write_admixed_summary(target,NL,targetdatasource=resultsdir,datasource=datasource,t.localanc=flocalanc,g.map,chrnos=chrnos)
    if (target!="simulated")
      write_admixed_summary(target,NL,targetdatasource=datasource,datasource=datasource,t.localanc=flocalanc,g.map,chrnos=chrnos)
